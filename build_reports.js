...
fs.writeFileSync('new_functions.js', updatedRefreshFunctions.replace(/\\\\/g, '').replace(/\\\\\$/g, '$'), 'utf8');

let html = fs.readFileSync('reports_fixed2.html', 'utf8');
let newJS = fs.readFileSync('new_functions.js', 'utf8');

const regex = /function refreshFinancialReport\(\) \{.*?(?=function showLoading)/s;

if (regex.test(html)) {
    html = html.replace(regex, newJS + '\n\n    ');
    fs.writeFileSync('reports.html', html, 'utf8');
    console.log('Successfully replaced functions and wrote to reports.html.');
} else {
